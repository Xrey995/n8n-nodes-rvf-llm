...
export const RVF_LLM_CHAT_MODEL_PROPERTIES: INodeProperties[] = [
	// 1. Провайдер (как в узле 1)
	{
		...
		description:
			'AI provider loaded from RVF LLM API. Only text-capable providers are shown.',
	},
	...
	// 2. Модель (как в узле 1)
	{
		...
		description:
			'Model loaded from RVF LLM API, filtered by provider and text capability.',
	},
	...
	// 3. Use Responses API (аналог OpenAI Chat Model)
	{
		...
		default: true,
		description:
			'If enabled, use the OpenAI-compatible /v1/responses endpoint instead of /v1/chat/completions.',
	},
	...
	// 5. Options – подмножество опций из узла 1 + флаг stream
	{
		...
		options: [
			...
			},
			{
				...
				default: false,
				description:
					'Stream the response if supported by the provider (server-side streaming).',
			},
			{
