...
class RvfLLMChatLangChain extends BaseChatModel {
   ...
   */
  private toolsToPrompt(tools: StructuredTool[]): string {
...
`.trim();
   ...
   */
  private parseToolCallFromText(text: string): { toolName: string; args: any } | null {
    if (!text) return null;
  ...
  async _generate(
    ...
    if (hasNativeToolCalls) {
      ...
      if (parsedTool) {
        ...
        aiMessage = new AIMessage(textContent);
      }
    } else {
   ...
   */
  bindTools(tools: StructuredTool[], kwargs?: any): this {
...
export class RvfLLMChatModel implements INodeType {
  description: INodeTypeDescription = {
    ...
    version: 1,
    description:
      'Use RVF LLM text models as chat models in your AI chains. Full tool calling support (native + emulated).',
    defaults: {
